...
Amino.prototype.unsubscribe = function(channel, handler) {
  this.invoke('pubsub', 'unsubscribe', channel, handler);
};
...
Amino.prototype.reset = function() {
  amino.globalAgent.initPool();
};

Amino.prototype.log = function() {
  if (this.get('debug')) {
    var args = Array.prototype.slice.call(arguments);
    var prefix = new Date().toUTCString() + ' - ';
    if (worker) {
